...
export function VideoPlayer({ videoId, autoplay = true, onClose }: VideoPlayerProps) {
  ...
  return (
    ...
    >
      {/* Oversized iframe (150% scale) cropped by the parent's overflow-hidden.
          This reliably pushes YouTube's own title/channel/share chrome
          (which YouTube renders at fixed pixel offsets from the video's
          true edges, not the container's) completely outside the visible
          area, regardless of the source video's native aspect ratio. */}
      <div
        ref={containerRef}
      ...
      >
          ...
          {onClose && (
            ...
            >
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path d="M2 2l12 12M2 14L14 2" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
              </svg>
            </button>
          )}
        </div>
        ...
        <button
          onClick={togglePlay}
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex h-16 w-16 items-center justify-center rounded-full bg-white/10 backdrop-blur-md text-white hover:bg-white/20 hover:scale-105 transition-all pointer-events-auto"
          aria-label={playing ? "Pause" : "Play"}
        >
